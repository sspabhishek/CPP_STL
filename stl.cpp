...
int main()
{
    ...
    //sort(a+2,a+4); //sorting for a specific range
   // array<int,12>a{1,23,3,4,2,6,21,44,43,3,45,53};
    int a[]={1,23,3,4,2,6,21,44,43,3,45,53};
    //cout<<a.size();
    //sort(a.begin(),a.end());
    //sort(a.begin()+6,a.end());
...
//     }
    ...
    sort(arr,arr+3,comp);
    //sort it accorrding to second element 
    ...
    cout<<cnt<<endl;
    long long num1= 244454874454418744;
    int cnt1= __builtin_popcountll(num1);
    cout<<cnt1<<endl;

    string s ="123"; //that is mendotary in sorted order \
    for getting all permutation other wise it will give in \
    dectionary order number of possibile permutation after that
    sort(s.begin(),s.end());
    do{
        cout<<s<<endl;
    }while(next_permutation(s.begin(),s.end()));

    int maxi = *max_element(a,a+12);
    int min = *min_element(a,a+12);
    cout<<maxi<<endl<<min;
